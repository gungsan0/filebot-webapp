...
| 소스 | API 키 | 대상 | 비고 |
|------|--------|------|------|
| **Wikidata** | 불필요 | 영화 + TV 시리즈 | 정식 제목/연도. 동일 제목 리메이크는 연도로 구분 |
| **TVmaze** | 불필요 | TV 에피소드 | 에피소드 제목까지 가져옴 |
| **TheMovieDB** | 필요 | 영화 + TV | `language`로 현지화 (예: `ko-KR`) |
| **OMDb** | 필요 | 영화 + 시리즈 | |

- 키가 필요한 소스(TheMovieDB/OMDb)는 UI의 API 키 칸에 키를 입력하면 동작한다.
- 키 없이 바로 쓰려면 **Wikidata**(영화)와 **TVmaze**(TV 에피소드)를 쓰면 된다.
